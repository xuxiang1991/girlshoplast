...
public class FspUserView extends RelativeLayout{
    ...
    public void onOneSecondTimer() {
        if (!hst.meetingdemo.utils.FspUtils.isEmptyText(m_userid)) {
            ...
            if (!hst.meetingdemo.utils.FspUtils.isEmptyText(m_videoid) || hst.meetingdemo.utils.FspUtils.isSameText(m_userid, fspM.getSelfUserId())) {
                VideoStatsInfo statsInfo = fspM.getVideoStats(m_userid, m_videoid);
                strInfo = String.format("%dx%d %dF %s", statsInfo.width, statsInfo.height, statsInfo.framerate, hst.meetingdemo.utils.FspUtils.convertBytes2HumanSize(statsInfo.bitrate));
            }
    ...
    private void checkUserState() {
        ...
        if (!m_haveAudio && hst.meetingdemo.utils.FspUtils.isEmptyText(m_videoid)) {
            m_userid = null;
